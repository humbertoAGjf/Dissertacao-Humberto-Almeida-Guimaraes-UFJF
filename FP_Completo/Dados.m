function [NBar, NLin, IndBar, BarGer, BarEMF, BarVTh, BTipo, PgEsp, PgeEsp, QgEsp, QgMax, QgMin, Pc, Qc, Qs, Ps, VEsp, ThEsp, DE, PARA, ...
    r, x, BSh_Lin, TapEsp, TapMin, TapMax, TapPh, MvaMax, LTipo, NGer, BarCGer, TapC, BarCTap, LadoCTap, FptTap, FptGerR, FptGerA, FptGerE, FreqEsp, Area, DArea, LoadDamping, FptHVDCE, DHVDC, LinhasHVDC, XhvdcEsp, DadoInc, DincCarga, DincGerador] = Dados(arquivo, GovernorControl)
    ...
    if (arquivo(end-3:end) == '.mat')
        ...
        if (exist('DELO','var') == 1 && exist('DCNV','var') == 1 && exist('DCCV','var') == 1)
            ...
            for i=1:LinhasHVDC
                ...
                DHVDC(i,21) = DELO(i,7);            % Modo do Elo. SE 0 = normal, se 1 = High Mvar Consumption, se 2 = Controle do artigo, se 3= controle convencional, se 4= stab50
                ...
                if ( DHVDC(i,31) == 0 && GovernorControl==1 && DHVDC(i,21) == 2)
                    fprintf("!!! ATENCAO !!!  PARA USAR ESSE MODO DE CONTROLE DE FREQUÊNCIA HVDC É PRECISO UTILIZAR A TENSÃO DE REFERÊNCIA NO INVERSOR!");
                    return
                end
        ...
        if (size(DArea,1) < Narea)
            IndA = sort(unique(Area));
            for i=1:size(DArea,1)
                IndA = IndA(IndA ~= DArea(i,1));
            end
            DArea = [DArea; [IndA 60*ones(length(IndA),1)]];
        end
